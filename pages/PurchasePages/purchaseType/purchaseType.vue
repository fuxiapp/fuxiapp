...
<script>
	...
	export default {
		...
		methods: {
			toPath (v) { // 选择类别
				if  (this.moduleType === 4 || this.moduleType === 5) { // 采购发货单/采购退货单
					this.salesSend.type = v.Name;
					this.salesSend.typeCode = v.Name;
					this.$API.setStorage('fuxiSalesSend', this.salesSend);
					if (this.salesSend.goodsId !== '' && this.salesSend.goodsId !== undefined) { // 一键收货
						this.$API.rto(`../../PurchasePages/editGoods/editGoods?id=${this.salesSend.goodsId}&type=${this.moduleType}&getType=1`);
					} else {
						this.$API.rto(`../../sale/invoice/invoice?id=${v.Type}&type=${this.moduleType}`);
					}
				}
			},
			getList() { // 获取销售列表
				this.$API.get('/fuxi/select/query-purchase-Type').then(res => {
					uni.hideLoading();
					if (res.code === 'success') {
