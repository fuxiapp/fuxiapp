<template>
	<view class="v-sel-customer">
		<view class="search" v-if="!isShowAddCustomer">
			<search placeholderStr="选择厂商名称/手机号" type="1"></search>
		</view>
		<view class="list-con">
			<purchaseItem :list="infoList" @toPath="toStrore"  type="1"></purchaseItem>
		</view>
		<!-- 新增客户 -->
...
<script>
	...
	export default {
		...
		methods: {
			toStrore (id) { // 选择店仓
				this.$API.to(`../../sale/selStore/selStore?type=${this.moduleType}&id=${id}`);
			},
			selCity (type) {
				this.cityType = type;
				if (type === 1) { // 获取省份
