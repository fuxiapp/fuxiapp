...

{
  ...
  methods: {
    ...
    okRadioValue: function okRadioValue(val) {// 确定选择类别
      this.isShowType = false;
    ...
    uploadImg: function uploadImg() {var _this = this; // 上传图片
      this.$API.upload().then(function (res) {
        _this.goodImg = res.imageSrc;
      });
    },
    opentType: function opentType(index) {var _this2 = this; // 打开筛选类型
      ...
      if (index === 1) {
        ...
        this.$API.get('/fuxi/select/query-goods-type').then(function (res) {
          if (res.code === 'success') {
            ...
            for (var i = 0; i < arr.length; i++) {
              var info = { id: arr[i].goodsTypeId, name: arr[i].goodsType, goodsTypeId: arr[i].goodsTypeId, flg: false };
              _this2.classTypeInfo.push(info);
            }
        ...
        this.$API.get('/fuxi/select/query-brand').then(function (res) {
          if (res.code === 'success') {
            var arr = res.data;
            for (var i = 0; i < arr.length; i++) {
              var info = { id: arr[i].brandId, name: arr[i].brand, flg: false };
              _this2.brandTypeInfo.push(info);
            }
        ...
        this.$API.get('/fuxi/select/query-age').then(function (res) {
          if (res.code === 'success') {
            var arr = res.data;
            for (var i = 0; i < arr.length; i++) {
              var info = { id: arr[i].age, name: arr[i].age, flg: false };
              _this2.ageTypeInfo.push(info);
            }
          }
        });
      } else if (index === 4) {
        ...
        this.$API.get('/fuxi/select/query-season').then(function (res) {
          if (res.code === 'success') {
            var arr = res.data;
            for (var i = 0; i < arr.length; i++) {
              var info = { id: arr[i].goodsTypeId, name: arr[i].season, flg: false };
              _this2.seasonTypeInfo.push(info);
            }
        ...
        this.$API.get('/fuxi/select/query-supplier').then(function (res) {
          if (res.code === 'success') {
            var arr = res.data;
            for (var i = 0; i < arr.length; i++) {
              var info = { id: arr[i].supplierId, name: arr[i].supplier, flg: false };
              _this2.supplierTypeInfo.push(info);
            }
        ...
        this.$API.get('/fuxi/select/query-color').then(function (res) {
          if (res.code === 'success') {
            var arr = res.data;
            for (var i = 0; i < arr.length; i++) {
              var info = { id: arr[i].colorId, name: arr[i].color, flg: false };
              _this2.colorTypeInfo.push(info);
            }
    ...
    save: function save() {var _this3 = this;
      this.$API.post(' /fuxi/goods/add-goods', { goodsDTO: this.goodsInfo }).then(function (res) {
        if (res.code === 'success') {
          uni.showToast({
            title: '保存成功!' });

          _this3.$API.tab('../../tab/main/main');
        }
      });
    } },
