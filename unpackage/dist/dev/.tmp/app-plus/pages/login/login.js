...
var _default =
{
  ...
  methods: {
    ...
    getDevlop: function getDevlop() {var _this2 = this;
      var para = { no: this.username };1;
      this.$API.get('/fuxi/login/query-user-department', para).then(function (res) {
        if (res.code === 'success') {
    ...
    login: function login() {var _this3 = this; // 登录成功
      if (this.btnDisabled) {
        console.log(23232, " at pages\\login\\login.vue:123");
        var userInfo = { no: this.username, password: this.password, hostName: this.platform, regUserId: this.regUserId };
        this.$API.get('/fuxi/login/login', userInfo, 'POST').then(function (res) {
          if (res.code === 'success') {
            _this3.userInfo = res.data;
            _this3.$API.setStorage('fuxiUserInfo', res.data);
            uni.showToast({
              title: '登录成功,即将跳转...',
              icon: 'none',
              duration: 2500,
